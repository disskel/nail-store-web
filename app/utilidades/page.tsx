...
/**
 * MÓDULO DE UTILIDADES Y RENTABILIDAD - v1.2.5
 * MANTIENE: Dashboard original con KPIs e Historial de 5 columnas.
 * INTEGRA: Gestión de Alertas Recurrentes (Activar, Desactivar, Eliminar).
 */
export default function UtilidadesPage() {
  ...
  // --- 4. MOTOR DE CARGA INTEGRADO ---
  const cargarTodo = async () => {
    setCargando(true);
    try {
      // Cargamos el reporte financiero y las reglas de pago en paralelo
  ...
  // --- 5. ACCIONES DE GESTIÓN DE OBLIGACIONES ---
  ...
  if (cargando) return <div className="flex h-screen items-center justify-center bg-zinc-950 text-indigo-500 font-black animate-pulse">SINCRONIZANDO DATA FINANCIERA...</div>;
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-4">
          <button 
            ...
            className="bg-zinc-800 hover:bg-zinc-700 text-white px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest border border-zinc-700 transition-all"
          >
            📅 Programar Pagos
          </button>
          <button 
      ...
      {/* TABLA DE HISTORIAL ORIGINAL (5 COLUMNAS) */}
      ...
      {/* --- NUEVO: TABLA DE GESTIÓN DE OBLIGACIONES --- */}
              ...
              {obligaciones.map((ob) => (
                <tr key={ob.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-6 text-right">
                    <button onClick={() => borrarRegla(ob.id)} className="text-zinc-600 hover:text-red-500 transition-colors">Eliminar 🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* --- MODALES RESGUARDADOS --- */}
      
      {/* 1. REGISTRO GASTO MANUAL */}
      {showModalGasto && (
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
          <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3.5rem] w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-5">
              <input placeholder="NOMBRE (EJ: RECIBO LUZ)" className="w-full bg-black border border-zinc-800 p-5 rounded-2xl text-white font-black uppercase" onChange={(e) => setNuevaOb({...nuevaOb, descripcion: e.target.value.toUpperCase()})} />
              <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={() => setShowModalConfig(false)} className="w-full text-zinc-500 font-black uppercase text-[10px] mt-4 text-center">Cerrar</button>
            </div>
          </div>
        </div>
      )}
